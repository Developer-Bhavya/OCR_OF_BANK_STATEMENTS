...
def get_images_by_prefix(prefix_list, num_images):
    """Get images that match any of the given prefixes"""
    try:
...
def is_similar(text, variations, threshold=70):
    """Check if text is similar to any of the variations using fuzzy matching"""
...
def get_highest_lowest_values(image_data, fields):
    """Calculate highest and lowest values for each field"""
    analysis = {}
    for field in fields:
        values = []
...
def analyze_images(folder_name, num_images, chart_type):
    """Main function to analyze images and extract data"""
    ...
    if not image_urls:
        return "No images found", None, None, "No data to display", "No comparison data available"
...
def clear_outputs():
    """Function to clear all outputs and reset inputs"""
    return (
        None,  # folder_input
        3,    # num_images
        "Bar Chart",  # chart_type
        "",   # status
        None, # gallery
        "",   # table_output
        None, # chart_output
...
""") as demo:
    ...
    analyze_btn.click(
        ...
        outputs=[status, gallery, chart_output, table_output, comparison_output]
    )
    ...
    clear_btn.click(
        ...
        outputs=[folder_input, num_images, chart_type, status, gallery, 
                table_output, chart_output, comparison_output]
    )
